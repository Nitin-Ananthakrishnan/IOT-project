...
// --- SAFETY THRESHOLDS ---
#define TRIP_CURRENT_mA 3000.0 
#define TRIP_TEMP_C     80.0   
...
void taskSensorsCode(void * parameter) {
  for(;;) {
    ...
    if (ntc_raw < 10 || ntc_raw > 4085) {
      ...
      float steinhart;
      steinhart = ntc_resistance / NTC_NOMINAL_RESIST;      
    ...
    if (xSemaphoreTake(dataMutex, portMAX_DELAY) == pdTRUE) {
      ...
      sharedData.motor_temp_c = ntc_temp_c; 
      ...
      sharedData.ntc_ok = local_ntc_ok; 
...
void taskCommsCode(void * parameter) {
  Serial.println("Core 0 Initialized. Waiting for network...");
  ...
  for(;;) {
    ...
    // 2. SECURED MQTT RECONNECT with LWT
    ...
    // 4. PHYSICAL LED STATUS
    bool local_fault = false, local_trip = false;
    if (xSemaphoreTake(dataMutex, 10) == pdTRUE) {
        local_fault = (!sharedData.ina_ok || !sharedData.bme_ok || !sharedData.mq_ok || !sharedData.ntc_ok);
        local_trip = sharedData.is_tripped;
        xSemaphoreGive(dataMutex);
    }
    digitalWrite(PIN_LED_TRIP, local_trip ? HIGH : LOW);
    digitalWrite(PIN_LED_RED, (local_fault && !local_trip) ? HIGH : LOW);
